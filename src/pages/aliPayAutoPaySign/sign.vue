<template>
    <div v-title="'送你20元停车优惠券'">
        <section id="wrapper" class="wrapper">
            <div class="pic">
                <img src="./images/reproduction.png" alt="">
                <img src="./images/reproduction2.png" alt="">
            </div>
            ...
            <div class="content" :class="{'magrinTop':!isSubscribe}">
                <h2><span>活动规则</span><img class="showMore" src="./images/jiantou.png" alt=""></h2>
                <p>1、活动期间，开通支付宝免密支付，即可获得20元停车优惠券；</p>
                <p>2、享受优惠后，停车费最低支付金额为0.01元；</p>
                <p>3、用户参与流程：关注停简单生活号/下载停简单APP→开通支付宝免密支付→领20元停车优惠券；</p>
                <p>4、每个用户只能领取一次；</p>
                <p>5、活动时间：2018年8月8日-2018年8月24日；</p>
                <p>6、活动最终解释权归停简单所有。咨询电话：<a :href="'tel:'+servicePhone">{{servicePhone | parseServicePhone}}</a></p>
            </div>
            <div class="couponUseRule" v-show="isSubscribe">
                <p class="ruleTitle">
                    <span class="titleContent">优惠券使用说明</span>
                </p>
                <ul class="ruleContent">
                    <li>1、20元停车立减券，分4次使用，每笔停车订单可抵停车费，车辆出场时会自动使用优惠券抵扣相应停车费；</li>
        ...
        methods: {
            sign(){
                if(util.isDispatchPushFromApp()){
                    util.cnzzSend("立即开通免密按钮APP",'点击')
                }else {
                    util.cnzzSend("支付宝生活号开通按钮",'点击')
                }
                ...
                if (this.hasBind){
                    ...
                    this.getActivityState().then(()=>{
                        if(this.isTemp){
                            util.setSession('isFromAlipayAutoPaySign','1')
                            util.cnzzSend("跳转到了匿名用户登陆页",'跳转')
                            location.href = util.constants.tcapiWebUrl + 'login/login';
                        }else{
            ...
            realSign(type){
                ...
                if(type == 'aliPay'){
                    signType = '3005';
                    if(util.isDispatchPushFromApp()){
                        returnUrl= util.constants.tcapiWebUrl + 'aliPayAutoPaySign/signSuccess?dispatchFrom=app';
                    }else {
                        returnUrl= util.constants.tcapiWebUrl + 'aliPayAutoPaySign/signSuccess';
                    }
                }
                tjdServices.request({
                    method:'autoPaySign',
